...
# 获取地址列表
@route_api.route("/my/address/index", methods=["GET", "POST"])
def myAddressList():
...
def myAddressOps():
    ...
    if act == "del":
        ...
        MemberAddress.query.filter_by(member_id=member_info.id).update({'is_default': 0})       # 全部信息改为无默认
        address_info.is_default = 1                                                             # 将当前设定为默认选中状态1
        address_info.updated_time = getCurrentDate()
        db.session.add(address_info)
        db.session.commit()
