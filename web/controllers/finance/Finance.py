...
def index():
    ...
    return i_render_template("finance/index.html", resp_data)
...
def info():
    ...
    resp_data['current'] = 'index'
    return i_render_template( "finance/pay_info.html",resp_data )
...
def set():
    ...
    stat_info = db.session.query(PayOrder, func.sum(PayOrder.total_price).label("total")).group_by(PayOrder).first()
    ...
    return i_render_template("finance/account.html", resp_data)
