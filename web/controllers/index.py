...
def index():
    ...
    date_from = getFormatDate(date=date_before_30days, format="%Y-%m-%d")       # 30天之前日期
    date_to = getFormatDate(date=now, format="%Y-%m-%d")                        # 当前的日期
    ...
    return i_render_template("index/index.html", resp_data)
